...
class DmControlWrapper(core.Env):
    ...
    def getObservation(self):
        self.timestep.observation['joint_angles'] = self.dmcenv.physics.joint_angles()
        self.timestep.observation['head_height'] = self.dmcenv.physics.head_height()
        self.timestep.observation['extremities'] = self.dmcenv.physics.extremities()
        self.timestep.observation['torso_vertical'] = self.dmcenv.physics.torso_vertical_orientation()
        self.timestep.observation['com_velocity'] = self.dmcenv.physics.center_of_mass_velocity()
        self.timestep.observation['velocity'] = self.dmcenv.physics.velocity()
    ...
    def _render(self, mode='human', close=False):
        if close:
            ...
            return
        if mode == 'human':
            self.pixels = self.dmcenv.physics.render(480, 640, camera_id=0)
